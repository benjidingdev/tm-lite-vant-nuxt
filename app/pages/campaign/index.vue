...
<template>
  <div class="w-full h-screen overflow-y-scroll">
    <div class="w-full text-white bg-black bg-[url('/campaign/page_bg_mobile.png')] lg:bg-[url('/campaign/page_bg.jpg')] bg-contain lg:bg-[length:auto_100%] lg:bg-center bg-no-repeat lg:pt-20 lg:pb-66 pt-14 pb-10">
      ...
      <van-row class="flex justify-center mt-8 text-[30px] font-black">
        <van-highlight class="text-white text-center px-14" highlight-class="!text-[#B3FF26]" source-string="下有效限價單，登上排行榜，贏取每日 $TUIT 獎勵。"
          keywords="每日 $TUIT" />
      </van-row>
      ...
      <van-row class="flex justify-center mt-7.5 text-[20px] text-center">
        <van-button class="font-roboto h-11 text-lg lg:!h-14 lg:!text-[24px] !font-black !rounded-full !px-10" @click="rulesVisible = true">活动规则</van-button>
        <van-button class="font-roboto h-11 text-lg lg:!h-14 lg:!text-[24px] !font-black !rounded-full !px-6 !ml-4 !text-white !bg-black !border-0"
          icon="/campaign/x.svg">分享到推特</van-button>
      </van-row>
      <div class="w-[90%] lg:w-[1222px] lg:px-[152px] px-5 backdrop-blur-xs bg-[#11111199] bg-[url('/campaign/card_bg_mobile.png')] lg:bg-[url('/campaign/card_bg.svg')] bg-[length:100%_100%] bg-no-repeat rounded-4xl mx-auto lg:mt-15 mt-74">
        <van-row class="font-roboto flex justify-center font-bold text-2xl lg:text-[36px] lg:pt-12 pt-9">
          完成已下步骤参与活动
        </van-row>
        <van-row class="mt-4 lg:mt-8">
          <CampaignSteps :value="['登录/注册账号', '分享到推特', '挂单']" :active="active" />
        </van-row>
        <van-divider dashed />
        ...
        <van-row class="font-roboto justify-center text-center font-bold text-lg text-[#03FB75] pt-6 pb-2 lg:pt-8 lg:pb-9 px-10">
      ...
      <div class="w-[90%] lg:w-[1192px] px-8 backdrop-blur-xs bg-[#11111199] bg-[url('/campaign/card_bg_mobile.png')] lg:bg-[url('/campaign/card_bg.svg')] bg-[length:100%_100%] bg-no-repeat rounded-4xl mx-auto lg:mt-88 mt-4 lg:pb-6 pb-10">
        <van-row class="font-roboto justify-center font-bold text-[32px] pt-9.5 pb-10 lg:pt-15 lg:pb-13 uppercase">
          Leaderboard
        </van-row>
        <van-row class="lg:w-[88%] lg:h-48 h-35 bg-bottom lg:bg-auto bg-[url('/campaign/ranking.svg')] bg-[length:100%_auto] bg-no-repeat mx-auto lg:mt-20">
          <van-col class="relative w-1/3" v-for="(item, ind) in 3">
            <van-col class="absolute lg:bottom-8 w-full text-center" :class="{'bottom-[-6px] lg:!bottom-12': ind === 1}">
              <van-row class="font-dinpro justify-center text-2xl font-bold lg:pb-4 pb-2" :class="{'!pb-0': ind === 1}">{{ ind + 1 }}</van-row>
              <van-row v-if="ind === 1" class="justify-center mb-1">
                <van-image class="w-5 h-4 lg:!w-7 lg:!h-5" src="/campaign/champion.svg" />
              </van-row>
              <van-image class="w-15 h-15 lg:!w-24 lg:!h-24 border-4 border-[#FFFFFF33] rounded-full" src="/icons/Avalanche.svg" />
              <van-row class="lg:items-begin items-center justify-center lg:mt-8 mt-1" :class="{'lg:!mt-10 mt-2': ind === 1}">
                <van-col>
                  <van-image class="w-6 h-5.5 lg:w-[58px] lg:h-[46px]" :src="`/campaign/0${ind + 1}.svg`" />
                </van-col>
                <van-col class="lg:pl-4 pl-1">
                  <van-row class="text-xs lg:text-base opacity-60">Avalanche</van-row>
                  <van-row class="font-dinpro text-xl">900</van-row>
                </van-col>
              </van-row>
            </van-col>
          </van-col>
        </van-row>
        <van-row class="h-12 leading-12 font-bold rounded-2xl bg-gradient-to-tr from-[#1652F033] to-[#7000FF] lg:mt-4 mt-8">
        ...
        <van-list>
          <van-row class="h-16 leading-16 font-bold rounded-xl" :class="{'bg-[#FFFFFF0A]': ind % 2 === 1 }"
            v-for="(item, ind) in 7" :key="ind">
            <van-col class="!w-8 h-8 leading-8 text-center text-sm rounded-full bg-[#FFFFFF33] my-auto lg:ml-16 ml-4 mr-6">4</van-col>
            <van-col class="w-1/2 font-normal !flex">
              <van-image class="!w-10 !h-10 rounded-full my-auto mr-4" src="/icons/Avalanche.svg" />
              <van-row>Avalanche</van-row>
            </van-col>
            <van-col>1400</van-col>
          </van-row>
        </van-list>
        <van-row class="h-16 leading-16 font-bold rounded-xl bg-[#3A5AFE]">
            <van-col class="!w-8 h-8 leading-8 text-center text-sm rounded-full bg-[#FFFFFF33] my-auto lg:ml-16 ml-6 mr-6">99+</van-col>
            <van-col class="w-1/2 font-normal !flex">
            ...
            <van-col v-if="token?.accessToken">0</van-col>
            <van-button v-else class="h-10">去登陆</van-button>
          </van-row>
      </div>
    </div>
  </div>
  <campaign-rules :visible.sync="rulesVisible" />
</template>
<style scoped>
::-webkit-scrollbar {
  display: none;
