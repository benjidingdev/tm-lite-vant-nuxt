...
<template>
<div class="h-[calc(100vh-var(--nav-height))] overflow-y-hidden flex flex-col">
    ...
    <van-card class="rounded-lg relative" v-for="item in voData.list" :key="item.adviseId">
      ...
      <template #desc>
        ...
        <div class="flex flex-row justify-between items-center pb-2">
          <div class="flex flex-row items-center">
            ...
            <p>{{ item.nickname }}</p>
            <van-icon v-if="voData.queryParams.myself === 1" @click="goShares(item)" name="share-o" size="15" class="ml-2"
              color="#1989fa" />
            </div>
      ...
      <template #footer>
        <div class="flex justify-between items-center">
          <a :href="useIpfsLink(item.hash)" v-if="item.hash" target="_blank">
            {{ shortenHash(item.hash, 4, 4) }}
