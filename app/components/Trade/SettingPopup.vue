...
<template>
  <van-popup position="bottom" v-model:show="modalIsShow.tradeSetting">
    <div class="text-center px-4 pt-4">
      <van-picker :value="tradeVolume" :columns="columns" :title="$t('Set Volume')" :confirm-button-text="$t('Confirm')"
        :cancel-button-text="$t('Cancel')" @confirm="setVolume" @cancel="setModal('tradeSetting', false)" />
    </div>
  </van-popup>
</template>
