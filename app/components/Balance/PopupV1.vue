...
<template>
  <van-dialog
    width="80vw"
    closeable
    v-model:show="modalIsShow.balanceModal"
    :title="$t('Transfer Crypto')"
    :showConfirmButton="false"
    :showCancelButton="false"
    :z-index="50"
  >
    <div
      class="text-center text-gray-400 border-b-[1px] border-gray-100 pb-3 mb-3"
    >
    ...
    <van-tabs>
      <van-tab :title="$t('Deposit')">
        <div class="text-sm px-4 pb-5 !max-h-[65vh] !overflow-y-scroll">
          ...
          <div
            class="flex md:items-center rounded-lg bg-[#3663891A] pl-4 py-2 mb-2"
          >
            <van-icon
              class="top-[1px]"
              name="info-o"
          ...
          <div
            class="md:flex justify-between sm:flex-row sm:gap-x-3 bg-[#0000000d] rounded-lg px-4 py-3"
          >
            <div class="w-full">
          ...
          <div class="md:flex justify-between mt-7 md:mt-11">
            ...
            <div>
              ...
              <div
                class="grid grid-cols-3 lg:grid-cols-6 gap-3 mt-7 md:mt-4 px-4 md:px-0"
              >
                <div
                  class="flex flex-col justify-center py-2.5 cursor-pointer bg-[#00000005] rounded"
                  :class="{ '!bg-[#0000001f]': ind == currentSite }"
                  v-for="(item, ind) in depositPlatforms"
                  @click="currentSite = ind"
                >
                  <div class="h-11 md:h-8 flex justify-center items-center">
                    <img
                      class="w-11 md:w-8"
                      :class="{ '!w-16 md:!w-12': item.name == 'Okx' }"
                      :src="item.icon"
                    />
                  </div>
